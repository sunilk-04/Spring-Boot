...
public class SolrController {
    ...
    private void connect() {
        this.client = SolrClientProvider.getInstance("http://" + this.host + ":" + this.port + "/solr/" + this.collection).getSolrClient();
    }
    ...
    @GetMapping("/docs")
    public List getAll(@Parameter(description = "Rows to return") @RequestParam(value = "rows", required = false, defaultValue = "20") Integer rows,
    ...
    @GetMapping("/docs/{id}")
    public SolrDocument get(@PathVariable String id) {
        try {
    ...
    public void delete(@PathVariable String id) throws SolrServerException, IOException {
    ...
    @PutMapping("/docs/{id}")
    public void update(@PathVariable String id, @RequestBody SolrDocument requestBody) throws SolrServerException, IOException {
        this.addOrUpdate(id, requestBody, true);
    }

    @Operation(summary = "Adds a document.", description = "Adds a document.")
    @PostMapping("/docs")
    public void add(@RequestBody SolrDocument requestBody) throws SolrServerException, IOException {
    ...
    public List query(@Parameter(description = "Query to perform (Format : field:value)", required = true) @RequestParam("q") String query,
